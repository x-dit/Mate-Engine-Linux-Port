Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/61100bfcce9b86440a912488042a5821"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)) + _Time.x * float4(0,8,0,0))[0.0];
				#else
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.9375551,0.7083759,1,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.9375551,0.7083759,1,1).a;
				#else
